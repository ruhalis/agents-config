...
For VS Code, or any clangd client, on an IDF project. Without this setup clangd flags every file. The cause is the compile database the build writes, `<app>/build/compile_commands.json`: it carries GCC-only `-f*` and `-m*` flags that clangd rejects (`unknown argument: '-mlongcalls'`, `'-fno-shrink-wrap'`), and clangd finds the newlib headers only when `--query-driver` lets it ask the cross compiler (otherwise `'sys/reent.h' file not found`, `'inttypes.h' file not found`). Build the app once so the database exists. Claude Code's clangd plugin starts Apple's `/usr/bin/clangd` with only `--background-index`, so an agent's LSP diagnostics in an IDF project stay wrong even after this setup; SKILL.md step 3 makes the build output the authority.
